...
Xh[0,:] = 1              # selects the entire first row of the mesh
Xh[n-1, :] = 2           # selects the entire  last row of the mesh
Xh[:, 0] = 1             # selects the entire first column of the mesh
Xh[:, n-1] = 2
...
for io in range(1, n-1,1):
    for jo in range(1, n-1,1):
        eq = (Xh[io+1, jo] + Xh[io-1, jo] + Xh[io, jo+1] + Xh[io, jo-1] - 4*(Xh[io,jo]))
        eqs.append(eq)
        sp.pretty_print(eq)
